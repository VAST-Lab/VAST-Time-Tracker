    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 px-4 py-2 rounded-md hover:bg-zinc-800 dark:hover:bg-white disabled:bg-zinc-300 dark:disabled:bg-zinc-700 disabled:dark:text-zinc-500 disabled:cursor-not-allowed transition-colors text-sm font-medium"
        >
          <Download size={16} />
          Export to CSV
        </button>
      ...
      <div className="bg-white dark:bg-zinc-900 p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <select value={selectedProject} onChange={e => setSelectedProject(e.target.value)} className="w-full rounded-md border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-950 px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100">
              ...
              <div key={idx} className="p-0">
                ...
                <ul className="divide-y divide-zinc-100 dark:divide-zinc-800">
                  {Object.values(project.users).map((user, uIdx) => (
                    <li key={uIdx} className="px-6 py-3 flex justify-between items-center hover:bg-zinc-50/50 dark:hover:bg-zinc-800/50">
                      <span className="text-sm font-medium text-zinc-700 dark:text-zinc-300">{user.userName}</span>
                      <span className="text-sm font-mono text-zinc-900 dark:text-zinc-100 bg-zinc-100 dark:bg-zinc-800 px-2 py-1 rounded">
